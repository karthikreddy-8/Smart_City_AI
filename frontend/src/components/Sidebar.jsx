    ...
    <>
      ...
      <aside className={`fixed md:static inset-y-0 left-0 z-50 w-64 bg-slate-900 text-white flex flex-col h-screen border-r border-slate-800 transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <div className="p-6 border-b border-slate-800 flex items-center space-x-3 shrink-0">
          <FaCity className="text-primary-500 text-3xl animate-pulse shrink-0" />
          <div className="min-w-0">
            <h1 className="font-extrabold text-lg bg-gradient-to-r from-primary-500 to-cyan-400 bg-clip-text text-transparent truncate">
              SmartCity AI
            </h1>
        ...
        {/* User Info */}
        {user && (
          <div className="p-4 mx-4 mt-4 bg-slate-800/50 rounded-xl border border-slate-800 flex items-center space-x-3 shrink-0">
            <div className="w-10 h-10 rounded-full bg-primary-500/20 border border-primary-500/40 flex items-center justify-center font-bold text-primary-400 shrink-0">
              {user.username[0].toUpperCase()}
            </div>
            <div className="min-w-0">
              <h4 className="text-sm font-semibold text-slate-200 truncate">{user.username}</h4>
              <span className="text-[10px] uppercase font-bold text-cyan-400 bg-cyan-950 px-2 py-0.5 rounded-full border border-cyan-800 inline-block mt-0.5">
                {role}
              </span>
            </div>
          </div>
