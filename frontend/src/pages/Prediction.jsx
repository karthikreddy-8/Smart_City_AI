...
import { 
  ...
  FaCheckCircle, FaCarSide, FaTemperatureHigh, FaSmog, FaCloudSun
} from 'react-icons/fa';
  ...
  useEffect(() => {
    ...
    const loadModels = () => {
      adminAPI.getModels().then(res => {
        ...
        // Only set default model on first load or if none is selected
        setSelectedModel(prev => prev || (active ? active.model_name : (res.data.length > 0 ? res.data[0].model_name : '')));
      }).catch(err => console.error("Could not fetch models:", err));
    };
    
    loadModels();
    interval = setInterval(loadModels, 10000); // Auto-refresh models
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-8">
      <div>
        <h1 className="text-3xl font-extrabold text-white flex items-center gap-3">
          <FaBrain className="text-primary-400" /> AI Predictions
        </h1>
        <p className="text-slate-400 mt-1">Simulate conditions and forecast traffic outcomes</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 glass rounded-3xl p-6 relative overflow-hidden">
          ...
          <form onSubmit={handlePredict} className="space-y-4 relative z-10">
            {models.length > 0 && (
              <div>
                <label className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1 block">AI Model Engine</label>
                <select 
                  className="w-full bg-slate-900/80 border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-primary-500"
                  value={selectedModel}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1 block">Vehicle Count</label>
                <div className="relative">
                  <FaCarSide className="absolute left-3 top-3.5 text-slate-500" />
                  <input type="number" name="vehicle_count" value={formData.vehicle_count} onChange={handleChange}
                    className="w-full bg-slate-900/80 border border-slate-700 rounded-xl pl-9 pr-4 py-3 text-white focus:ring-2 focus:ring-primary-500" />
                </div>
              </div>
              <div>
                <label className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1 block">Accident Count</label>
                <div className="relative">
                  <FaExclamationTriangle className="absolute left-3 top-3.5 text-slate-500" />
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1 block">Road Type</label>
              ...
              <div>
                <label className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1 block">Weather</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1 block">Temperature (°C)</label>
                <div className="relative">
                  <FaTemperatureHigh className="absolute left-3 top-3.5 text-slate-500" />
                  <input type="number" name="temperature" value={formData.temperature} onChange={handleChange}
                    className="w-full bg-slate-900/80 border border-slate-700 rounded-xl pl-9 pr-4 py-3 text-white focus:ring-2 focus:ring-primary-500" />
                </div>
              </div>
              <div>
                <label className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1 block">Humidity (%)</label>
                <div className="relative">
                  <FaSmog className="absolute left-3 top-3.5 text-slate-500" />
          ...
          <AnimatePresence mode="wait">
            {!result && !loading && (
              ...
              >
                ...
                <p className="max-w-xs">Adjust the simulation parameters on the left and click predict to see AI-generated insights.</p>
              </motion.div>
            )}
            ...
            {loading && (
              ...
              >
                ...
                <h3 className="text-xl font-bold text-white animate-pulse">Running Inference...</h3>
              </motion.div>
            )}
              ...
              >
                ...
                <div className="glass rounded-3xl p-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-slate-800/50 rounded-2xl p-4 border border-slate-700/50">
                        <p className="text-xs font-bold text-slate-400 uppercase mb-1">CO2 Avoided</p>
